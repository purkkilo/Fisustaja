  ...
  <div class="container">
    ...
    <div class="container-transparent">
      ...
      >
        ...
        <v-tab href="#fishes" :disabled ="!basic_info_validated">2. Kilpailun kalalajit</v-tab>
        <v-tab href="#points" :disabled ="!fish_species_validated">3. Pistekertoimet ja alamitat</v-tab>
        <v-tab href="#summary" :disabled ="!fish_specs_validated">4. Yhteenveto</v-tab>
      </v-tabs>
      ...
      <v-tabs-items v-model="tab" style="background: rgba(1,1,1,0.4);">
        ...
        <v-tab-item class="inputarea" :value="'basic-info'">
          <v-container v-if="!loading">
            <v-row>
              <v-col>
                <p class="center-align flow-text black--text">Kilpailun perustiedot</p>
                <p
              ...
              <v-col md="8" offset-md="2">
                ...
                <v-row v-else class="valign-wrapper">
                  <v-col md="6" offset-md="2">
                    <p class="flow-text">Ei luotuja cuppeja, Luo cup ensin "Jatka Kilpailuja sivulla" (Voit myös luoda cupin kilpailuille, jotka ei ole missään cupissa")</p>
                  </v-col>
                  <v-col md="2">
                    <router-link to="/continue">
                        <v-btn tile color="blue lighten-1"><i class="material-icons left">add_circle_outline</i>Luo cup!</v-btn>
                    </router-link>
                  </v-col>
                </v-row>
                <v-row v-if="cup.name">
                  <v-col md="6" offset-md="3" class="input-fields">
                    <v-text-field
                      ...
                      maxlength="6"
                      @click:append-outer="cup_placement_points >= 0 ? cup_placement_points++ : cup_placement_points = 1" 
                      prepend-icon="remove" 
                      @click:prepend="cup_placement_points >= 1 ? cup_placement_points-- : cup_placement_points = 0"
                      @paste.prevent
                      :counter="6"
                ...
                <v-row v-if="cup.name">
                  <v-col md="6" offset-md="3" class="input-fields">
                    <v-text-field
                      ...
                      append-outer-icon="add"
                      maxlength="6"
                      @click:append-outer="cup_participation_points >= 0 ? cup_participation_points++ : cup_participation_points = 1" 
                      prepend-icon="remove" 
                      @click:prepend="cup_participation_points >= 1 ? cup_participation_points-- : cup_participation_points = 0"
                      @paste.prevent
                      :counter="6"
                      @keypress="isNumber($event, true)"
                      :rules="number_rules"
                ...
                <v-row v-if="cup.name">
                  <v-col md="6" offset-md="3" class="input-fields">
                    <v-text-field
                      ...
                      append-outer-icon="add"
                      maxlength="3"
                      @click:append-outer="cup_points_multiplier >= 1 ? cup_points_multiplier += 0.5: cup_points_multiplier = 1" 
                      prepend-icon="remove" 
                      @click:prepend="cup_points_multiplier >= 1.5 ? cup_points_multiplier -= 0.5: cup_points_multiplier = 1"
                      @paste.prevent
                      :counter="3"
                      @keypress="isNumber($event, true)"
                      :rules="number_rules"
                ...
                <v-row v-if="cup.name">
                  <v-col md="6" offset-md="3" class="input-fields">
                        ...
                        <v-col offset-md="4">
                          <v-radio-group v-model="team_competition" row :disabled="basic_info_validated">
                ...
                <v-row v-if="cup.name">
                  <v-col cols="12" md="5">
                    <v-row align="center">
                      <span class="flow-text black-text col s6">Kilpailun Aloituspäivä</span>
                    </v-row>
                    <v-row align="center">
                      <v-date-picker
                        style="height:480px"
                  ...
                  <v-col cols="12" md="5" offset-md="2" style="margin-bottom:50px">
                      <v-row align="center">
                        <span class="flow-text black-text col s6">Kilpailun Lopetuspäivä</span>
                      </v-row>
                      <v-row>            
                        <v-date-picker
                          style="height:480px"
                ...
                <v-row v-if="cup.name">
                  <v-col cols="12" md="5">
                    <v-row align="center">
                      <v-col>
                        <span class="flow-text black-text">Kilpailun alkamismisaika</span>
                      </v-col>
                    </v-row>
                    <v-row align="center">
                      <v-col>
                  ...
                  <v-col cols="12" md="5" offset-md="1">
                    <v-row align="center">
                      <v-col md="12" offset-md="2">
                        <span class="flow-text black-text">Kilpailun loppumisaika</span>
                      </v-col>
                    </v-row>
                    <v-row align="center">
                      <v-col md="12" offset-md="2">
                ...
                <v-row v-if="!basic_info_validated">
                  <v-col md="6" offset-md="3" v-if="cup.name">
                    <v-btn large block color="green" @click="checkBasicInformation"><i class="material-icons left">check</i>Jatka</v-btn>
                  </v-col>
                </v-row>
              </v-col>
              <v-row v-if="basic_info_validated">
                <v-col md="6">
                  <v-btn large tile color="grey" @click="disableInputs(false)"><i class="material-icons left">settings</i>Muuta tietoja</v-btn>
                </v-col>
                <v-col md="6">
                  <v-btn large tile color="green" @click="changeTab('fishes')"><i class="material-icons right">arrow_forward</i>Jatka</v-btn>
                </v-col>
              </v-row>
            </v-row>
          </v-container>
          ...
          <v-container>
            ...
            <v-row style="margin-top:50px">
              <v-row v-if="!fish_species_validated">
                <v-col md="6">
                  <v-btn large tile color="blue lighten-2" @click="changeTab('basic-info')"><i class="material-icons left">arrow_back</i>Takaisin</v-btn>           
                </v-col>
                <v-col md="6">
                  <v-btn large tile color="green" @click="checkFishSpecies"><i class="material-icons left">check</i>Jatka</v-btn>           
                </v-col> 
              </v-row>
              <v-row v-else>
                <v-col>
                ...
                <v-col>
                  <v-btn large tile color="green" @click="changeTab('points')"><i class="material-icons right">arrow_forward</i>Jatka</v-btn>           
                </v-col>
              </v-row>
            </v-row>
          </v-container>
          ...
          <v-container>
            ...
            <v-row>
              <v-row v-if="selected.length && !validated">
                <v-col md="6">
                  <v-btn large tile color="blue lighten-2" @click="changeTab('fishes')"><i class="material-icons left">arrow_back</i>Takaisin</v-btn>
                </v-col>
                <v-col md="6">
                  <v-btn large tile color="green" @click="checkFishSpecs"><i class="material-icons left">check</i>Tarkista tiedot</v-btn>
                </v-col>
              </v-row>
              <v-row v-else>
                <v-col md="4">
                ...
                <v-col md="4">
                  <v-btn large tile color="green" @click="changeTab('summary')"><i class="material-icons right">arrow_forward</i>Jatka</v-btn>
                </v-col>
              </v-row>
            </v-row>
          </v-container>
            ...
            <v-row>
              <v-row>
                <v-col md="5" offset-md="1">
                  <v-btn large tile color="blue lighten-2" @click="changeTab('points')"><i class="material-icons left">arrow_back</i>Takaisin</v-btn>
                </v-col>
                <v-col md="5" v-if="validated">
                  <v-btn large tile color="green" @click="submitCompetition"><i class="material-icons left">add_circle_outline</i>Luo kilpailu näillä tiedoilla</v-btn>
                </v-col>
              </v-row>
            </v-row>             
          </v-container>
      ...
      data() {
        return {
          ...
          rules: [
            ...
            value => (value || '').length <= this.max_input || 'Enintään 40 merkkiä',
          ],
          number_rules: [
            value => !!value || 'Kenttä ei voi jäädä tyhjäksi!',
      ...
      methods: {
        ...
        checkBasicInformation: function() {
          ...
          if (!this.start_time || !isStartTimeValid) {
            !this.start_time == true
              ...
              : this.showError('Syötä aika muodossa "hh:mm" (esim: 13:00). Syötetty aika oli: ' + this.start_time);
          }
          if (!this.end_time || !isEndTimeValid) {
            !this.end_time == true
              ? this.showError("Kilpailun loppumisaika puuttuu!")
              : this.showError('Syötä aika muodossa "hh:mm" (esim: 13:00). Syötetty aika oli: ' + this.end_time);
          }
          ...
          if (!this.end_date || !isEndDateValid) {
            ...
            let start_date = moment(`${temp_start} ${this.start_time}`, 'DD.MM.YYYY HH:mm');
            let end_date = moment(`${temp_end} ${this.end_time}`, 'DD.MM.YYYY HH:mm');
        ...
        checkFishSpecs: function() {
          ...
          for (let i = 1; i < this.selected.length + 1; i++) {
            ...
            if (!fish_spec.multiplier || !fish_spec.minsize || parseInt(fish_spec.multiplier) < 1 || parseInt(fish_spec.minsize) < 0) {
              this.showError(`Syötä kalan '${fish_spec.name}' kaikki tiedot oikein! (Pistekerroin > 0, Alamitta >= 0 )`);
            } else {
