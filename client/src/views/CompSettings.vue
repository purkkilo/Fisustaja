...
export default {
  ...
  mounted() {
    ...
    if (localStorage.getItem("competition") != null && !this.competition) {
      const competition = JSON.parse(localStorage.getItem("competition"));
      const competition_id = competition["id"];
      this.refreshCompetition(competition_id);
    } else {
    ...
    async refreshCompetition(competition_id) {
      ...
      try {
        ...
        if (competitions.length) {
          ...
          this.formatted_start_date = `${this.start_date.date()}.${this.start_date.month() +
            1}.${this.start_date.year()}`;
          this.formatted_end_date = `${this.end_date.date()}.${this.end_date.month() +
            1}.${this.end_date.year()}`;
        } else {
